...
public interface JobOpeningMapper {
  ...
  void register(JobOpening jobOpening);

  void modify(int jobOpeningId);

  void delete(String userId, int jobOpenginId);

  // 채용공고 목록 가져오기
  List<JobOpening> findAll();

  // 채용공고 상세 페이지 불러오기
  JobOpening findById(int jobOpeningId, int companyId);
